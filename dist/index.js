...
// utils/check-environment-variables.ts
function checkEnvironmentVariables(strapiUrl) {
  if (!strapiUrl) {
    throw new Error(
...
// utils/strapi-client.ts
var STRAPI_BASE_URL = import.meta.env.STRAPI_BASE_URL || "http://localhost:1337";
var strapiClient = strapi({ baseURL: STRAPI_BASE_URL + "/api" });
checkEnvironmentVariables(STRAPI_BASE_URL);

// utils/fetch-from-strapi.ts
async function getCollectionType(name, params) {
  const data = await strapiClient.collection(name).find(params);
  return data;
}
async function fetchFromStrapi(collectionName, params) {
...
// utils/inferSchemaFromResponse.ts
function inferSchemaFromResponse(data, fieldName) {
  if (data === null) return z.null();
  if (data === void 0) return z.undefined();
  if (Array.isArray(data)) {
    if (data.length === 0) {
    ...
    if (hasComponents(data)) {
      ...
      if (validComponentGroups.length > 0) {
        try {
          ...
          return z.array(z.discriminatedUnion("__component", componentSchemas));
        } catch (error) {
...
function inferEmptyArrayTypeFromName(fieldName) {
  const patterns = [
...
function hasComponents(data) {
  return data.some(
    (item) => item && typeof item === "object" && "__component" in item
  );
}
function inferPrimitiveSchema(data) {
...
function strapiLoader({
  contentType,
  strapiUrl = import.meta.env.STRAPI_BASE_URL || "http://localhost:1337",
  syncInterval = 60 * 1e3,
  params = {},
  pageSize = 25
}) {
  ...
  return {
    ...
    load: async function({ store, meta, logger }) {
      ...
      try {
            ...
            if (item && item.id) {
              try {
                ...
                store.set({
                  ...
                  // Use the validated data from the schema
                });
        ...
        logger.error(
          `Error loading Strapi content: ${error.message}`
        );
        throw error;
      }
    },
    schema: async () => {
      ...
      console.log(
        "SCHEMA STRUCTURE:",
        schema._def ? `Root type: ${schema.constructor.name}` : "Unable to inspect schema"
      );
      return schema;
    }
  };
